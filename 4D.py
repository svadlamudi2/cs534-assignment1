...
def greedy4D(initialBoard):
    ...
    if not board.isSafe(board):
        for move in board.findPossibleMovesFromBoard4D():
...
def tallyResults4D(N):
    file = open("data4D.csv","w")
    ...
    for j in range(4,N):
        ...
        for i in range(0, N):
            print("Greedy:")
            st, board, nodeCount, level, cost = greedy4D(generateBoard(j, i))
            ...
            print("A*")
            st, board, nodeCount, level, cost = aStar4D(generateBoard(j, i))
            if board.isSafe(board):
                solvedA+=1
